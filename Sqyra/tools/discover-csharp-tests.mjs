...
function discover(name, filter) {
  ...
  if (result.status !== 0)
    throw new Error(result.stderr || result.stdout || `dotnet exited ${result.status}`);
  const xml = readFileSync(resolve(results, file), "utf8");
  const definitions = new Map(
    [...xml.matchAll(/(<UnitTest\b[^>]*>)([\s\S]*?)<\/UnitTest>/g)].map((match) => {
  ...
  const tests = [...xml.matchAll(/<UnitTestResult\b[^>]*>/g)]
    .map((match) => {
      const testId = attribute(match[0], "testId");
      const encodedName = attribute(match[0], "testName");
      if (!testId || !encodedName) throw new Error("Malformed UnitTestResult entry in TRX output.");
      const displayName = decode(encodedName);
      const rawBase = definitions.get(testId) ?? displayName;
      const base = rawBase.replace(/_[0-9A-F]{6,8}(?=_|$)/g, "_Case");
      const count = (occurrence.get(base) ?? 0) + 1;
      occurrence.set(base, count);
      return { id: `${base}#${count}`, displayName };
    })
...
try {
  ...
  console.log(
    Object.entries(data.groups)
      .map(([name, value]) => `${name}: ${value.tests.length}`)
      .join("\n"),
  );
} finally {
