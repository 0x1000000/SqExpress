...
const source = await readFile(
  resolve(root, "../Test/SqExpress.Test/SqlParser/TSqlParserPortableFunctionTest.cs"),
  "utf8",
);
...
const method =
  "SqExpress.Test.SqlParser.TSqlParserPortableFunctionTest.ParseUnknownScalarFunctions_MapToExprScalarFunction";
const pattern = /\[TestCase\(@"([^"]*)",\s*"([^"]+)",\s*(\d+)\)\]/g;
let count = 0;
for (const match of source.matchAll(pattern)) {
  const sql = match[1];
  const name = match[2];
  const argumentCount = Number(match[3]);
