...
const sourcePath = resolve(
  root,
  "../Test/SqExpress.Test/SqlParser/TSqlParserInvalidSyntaxAndInjectionTest.cs",
);
...
const pattern =
  /new TestCaseData\(\s*("(?:\\.|[^"\\])*")\s*,\s*("(?:\\.|[^"\\])*")\s*\)\s*\.SetName\("([^"]+)"\)/g;
const imported = [];
for (const match of source.matchAll(pattern)) {
  imported.push({
    id: `${prefix}${match[3]}#1`,
    sql: JSON.parse(match[1]),
    expectedErrorExact: JSON.parse(match[2]),
  });
}
if (imported.length < 70)
  throw new Error(`Expected at least 70 invalid-syntax cases, found ${imported.length}.`);
...
if (duplicateCrossJoin !== undefined)
  imported.push({ ...duplicateCrossJoin, id: `${prefix}CrossJoinWithOn#2` });
