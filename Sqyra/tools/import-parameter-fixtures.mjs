...
const shared = "SELECT [u].[UserId] FROM [dbo].[Users] [u] WHERE [u].[UserId]=@id AND [u].[Name]=@name";
const imported = [
  { id: `${prefix}ExtractParameters(\"select UserId from Users u wHeRe [u].[UserId]=@id and [u].[Name]=@name\")#1`, sql: "select UserId from Users u wHeRe [u].[UserId]=@id and [u].[Name]=@name" },
  { id: `${prefix}ExtractParameters(\"select UserId from Users wHeRe userId=@id and [Name]=@name\")#1`, sql: "select UserId from Users wHeRe userId=@id and [Name]=@name" },
  { id: `${prefix}NamedParametersArePreservedByFormatter#1`, sql: shared },
  { id: `${prefix}NamedParametersAreMappedToSqExpr#1`, sql: shared },
  { id: `${prefix}ParameterMarkersInsideStringLiteralArePreservedByFormatter#1`, sql: "SELECT [u].[UserId] FROM [dbo].[Users] [u] WHERE [u].[Name]='@id'" },
