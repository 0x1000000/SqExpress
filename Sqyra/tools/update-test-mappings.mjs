...
const groups = Object.fromEntries(
  Object.entries(discovered.groups).map(([name, group]) => [
    name,
    {
      tests: group.tests.map((test) =>
        fixtureIds.has(test.id)
          ? { sourceId: test.id, status: "mapped", fixtureId: test.id }
          : portBySource.has(test.id)
            ? { sourceId: test.id, status: "mapped", port: portBySource.get(test.id) }
...
writeFileSync(
  resolve(root, "baseline/test-mappings.json"),
