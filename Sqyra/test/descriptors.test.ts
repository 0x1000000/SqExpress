...
describe("table descriptors", () => {
  ...
  it("escapes physical names that collide with descriptor members", () => {
    const table = defineTable({ schema: "dbo", name: "Reserved", columns: { as: column(sqlType.int32), $metadata: column(sqlType.int32) } });
  ...
  it("creates independent aliases", () => {
    const left = users.as("u1"); const right = aliasTable(users, "u2");
    expect(left.id.sourceAlias).toBe("u1"); expect(right.id.sourceAlias).toBe("u2"); expect(users.id.sourceAlias).toBe("Users");
  ...
  it("acts as an immutable table-reference factory", () => {
    const automatic = users(); const explicit = users("U"); const manager = users("Manager");
