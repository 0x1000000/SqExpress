...
describe("generated AST", () => {
  ...
  it("dispatches visitors exhaustively", () => {
    ...
    const visitor = new Proxy(
      {},
      { get: (_, kind: string) => (value: { kind: string }) => `${kind}:${value.kind}` },
    ) as AstVisitor<string>;
  ...
  it("rejects missing and extra fields at runtime", () => {
    expect(() => Reflect.apply(exprInt32Literal, null, [{}])).toThrow(
      "requires exactly these fields",
    );
    expect(() => Reflect.apply(exprInt32Literal, null, [{ value: 1, extra: true }])).toThrow(
      "requires exactly these fields",
    );
  });
});
