...
import { createVisitor, deserializeAst, descendants, exprBooleanAnd, exprBooleanEq, exprInt32Literal, exprPortableScalarFunction, exprStringLiteral, find, isAstNode, modify, parseTSql, serializeAst, toSql, visit, walk, walkWithParent } from "../src/index.js";
...
describe("AST operations", () => {
  it("walks pre-order and visits shared nodes by occurrence", () => {
    expect([...walk(tree().root)].map((n) => n.kind)).toEqual(["ExprBooleanAnd", "ExprBooleanEq", "ExprInt32Literal", "ExprInt32Literal", "ExprBooleanEq", "ExprInt32Literal", "ExprInt32Literal"]);
  });
  ...
  it("preserves identity for a no-op and rebuilds only changed paths", () => {
    ...
    const changed = modify(value.root, (node) => node === value.one ? exprInt32Literal({ value: 3 }) : node);
  ...
  it("validates kinds, required fields, and child categories", () => {
    ...
    expect(isAstNode({ kind: "ExprBooleanAnd", left: value.one, right: value.equality })).toBe(false);
  ...
  it("serializes bigint values deterministically", () => {
    expect(serializeAst(exprInt32Literal({ value: 1 }))).toBe('{"kind":"ExprInt32Literal","value":1}');
  });
  it("reports exact parents, depths, and root-to-node paths", () => {
    const value = tree(); const entries = [...walkWithParent(value.root)];
    expect(entries.map(({ node, parent, depth }) => [node.kind, parent?.kind ?? null, depth])).toEqual([
      ["ExprBooleanAnd", null, 0], ["ExprBooleanEq", "ExprBooleanAnd", 1],
      ["ExprInt32Literal", "ExprBooleanEq", 2], ["ExprInt32Literal", "ExprBooleanEq", 2],
      ["ExprBooleanEq", "ExprBooleanAnd", 1], ["ExprInt32Literal", "ExprBooleanEq", 2],
      ["ExprInt32Literal", "ExprBooleanEq", 2],
    ]);
    expect(entries[2]?.path.map((node) => node.kind)).toEqual(["ExprBooleanAnd", "ExprBooleanEq", "ExprInt32Literal"]);
  });
  it("round-trips deterministic JSON and rejects malformed serialized nodes", () => {
    const value = tree().root; const restored = deserializeAst(serializeAst(value));
    expect(restored).toEqual(value); expect(Object.isFrozen(restored)).toBe(true);
    expect(() => deserializeAst('{"kind":"ExprBooleanAnd","left":null,"right":null}')).toThrow(/invalid/i);
  });
  it("rejects removal of required children and incompatible replacements", () => {
    const value = tree();
    expect(() => modify(value.root, (node) => node === value.one ? null : node)).toThrow(/required child/);
    expect(() => modify(value.root, (node) => node === value.equality ? value.one : node)).toThrow(/Invalid replacement/);
  });
  it("dispatches exhaustive visitors by concrete node kind", () => {
    const calls: string[] = [];
    const visitor = createVisitor((node) => { calls.push(node.kind); return node.kind; });
  ...
  it("traverses, modifies, and serializes STRING_AGG nodes", () => {
    ...
    const changed = modify(ast, (node) => node.kind === "ExprStringLiteral" && node.value === "old" ? exprStringLiteral({ value: "new" }) : node);
    expect(changed).not.toBeNull();
    expect(toSql(deserializeAst(serializeAst(changed!)), { dialect: "tsql" })).toBe("SELECT STRING_AGG('new','|') WITHIN GROUP (ORDER BY 2 DESC)");
  });
  it("traverses and round-trips portable JSON AST nodes", () => {
    ...
    expect(kinds).toContain("ExprQueryAsJson"); expect(kinds).toContain("ExprJsonValue");
    expect(deserializeAst(serializeAst(ast))).toEqual(ast);
  });
  it("round-trips portable scalar function nodes", () => {
    const node = exprPortableScalarFunction({ portableFunction: "Len", arguments: [exprStringLiteral({ value: "abc" })] });
