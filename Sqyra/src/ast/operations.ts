import {
  ...
  type AstVisitor,
  type Expr,
} from "./generated/ast.generated.js";
...
export function createVisitor<R, C = void>(
  fallback: (node: Expr, context: C) => R,
  overrides: Partial<AstVisitor<R, C>> = {},
): AstVisitor<R, C> {
  const visitor = new Proxy(overrides, {
    get(target, property) {
...
export function isAstNode(
  ...
  for (const field of nodeFields[record.kind]) {
    ...
    if (!field.child) {
      if (field.collection) {
        if (field.expectedType === "Byte") {
          if (!(fieldValue instanceof Uint8Array)) return false;
        } else if (
          !Array.isArray(fieldValue) ||
          !fieldValue.every((item) => isScalar(item, field.expectedType))
        )
          return false;
    ...
    for (const child of values)
      if (
        !isAstNode(child, nextAncestors) ||
        field.expectedType === null ||
        !nodeTypeKinds[field.expectedType]?.has(child.kind)
      )
        return false;
  }
  return true;
...
export interface WalkEntry {
  readonly node: Expr;
  readonly parent: Expr | null;
  readonly depth: number;
  readonly path: ReadonlyArray<Expr>;
}
export function* walkWithParent(root: Expr): IterableIterator<WalkEntry> {
  ...
  function* inner(node: Expr, parent: Expr | null): IterableIterator<WalkEntry> {
    ...
    for (const field of childFields[node.kind]) {
      ...
      if (value === null) continue;
      if (field.collection)
        for (const child of value as ReadonlyArray<Expr>) yield* inner(child, node);
...
export function* descendants(root: Expr, includeSelf = false): IterableIterator<Expr> {
  ...
  for (const node of walk(root)) {
    if (includeSelf || !first) yield node;
    first = false;
  }
}

export type ExprOfKind<K extends AstKind> = Extract<Expr, { readonly kind: K }>;

export function* descendantsOfType<K extends AstKind>(
  root: Expr,
  kind: K,
  ...
  for (const node of descendants(root)) {
    ...
    const typedNode = node as ExprOfKind<K>;
    if (predicate === undefined || predicate(typedNode)) yield typedNode;
  }
}
...
export function modify(root: Expr, modifier: (node: Expr) => Expr | null): Expr | null {
  ...
  function inner(node: Expr): Expr | null {
    ...
    for (const field of childFields[node.kind]) {
      ...
      if (value === null) continue;
      if (field.collection) {
        const original = value as ReadonlyArray<Expr>;
        const replacement: Expr[] = [];
        let collectionChanged = false;
        for (const child of original) {
          const next = inner(child);
          if (next !== child) collectionChanged = true;
          if (next !== null) {
            ensureType(next, field.expectedType, node.kind, field.name);
        ...
        const original = value as Expr;
        const next = inner(original);
        if (next === null && !field.nullable)
          throw new Error(`Cannot remove required child ${node.kind}.${field.name}`);
        if (next !== original) {
          changed = true;
          if (next !== null) ensureType(next, field.expectedType, node.kind, field.name);
    ...
    const rebuilt = changed
      ? freezeNode(
          node.kind,
          fields,
          nodeFields[node.kind].map((field) => field.name),
        )
      : node;
...
function isScalar(value: unknown, type: string): boolean {
  ...
  switch (type) {
    ...
    case "Int64":
      return (
        typeof value === "bigint" && value >= -9223372036854775808n && value <= 9223372036854775807n
      );
    case "Double":
    ...
    case "DateTime":
      return (
        isRecord(value) &&
        typeof value.value === "string" &&
        ["unspecified", "utc", "local"].includes(String(value.kind))
      );
    case "DateTimeOffset":
      return isRecord(value) && typeof value.value === "string";
    case "DecimalPrecisionScale":
      return isRecord(value) && Number.isInteger(value.precision) && Number.isInteger(value.scale);
    default:
      return false;
  }
}
...
function ensureType(node: Expr, expected: string, parent: AstKind, field: string): void {
  if (!nodeTypeKinds[expected]?.has(node.kind))
    throw new Error(
      `Invalid replacement ${node.kind} for ${parent}.${field}; expected ${expected}`,
    );
}
...
export function deserializeAst(json: string): Expr {
  const revive = (value: unknown): unknown => {
    ...
    if (typeof value.kind !== "string" || !isAstKind(value.kind))
    ...
    for (const field of nodeFields[value.kind]) {
      if (!(field.name in value))
        throw new TypeError(`Serialized ${value.kind} is missing ${field.name}.`);
      fields[field.name] = revive(value[field.name]);
    }
    const node = freezeNode(
      value.kind,
      fields,
      nodeFields[value.kind].map((field) => field.name),
    );
    if (!isAstNode(node))
      throw new TypeError(`Serialized ${value.kind} has an invalid field value.`);
    return node;
  };
...
export function normalizeAstForCSharpFixture(root: Expr): Readonly<Record<string, unknown>> {
  const normalizeNode = (node: Expr): Readonly<Record<string, unknown>> => {
    ...
    const result: Record<string, unknown> = {
      $type: node.kind.startsWith("ExprExpr") ? node.kind.slice(4) : node.kind.slice(4),
    };
    for (const field of nodeFields[node.kind]) {
      ...
      if (
        ...
        value !== null &&
        "kind" in value &&
        value.kind === "ExprQuerySpecification" &&
        "selectList" in value &&
        Array.isArray(value.selectList) &&
        value.selectList.length === 0
      )
        continue;
      const name = `${field.name[0]!.toUpperCase()}${field.name.slice(1)}`;
      if (field.child)
        result[name] = field.collection
          ? (value as ReadonlyArray<Expr>).map(normalizeNode)
          : normalizeNode(value as Expr);
        ...
        result[name] = (value as { readonly value: unknown }).value;
      else if (
        typeof value === "object" &&
        value !== null &&
        field.expectedType === "DecimalPrecisionScale"
      )
        for (const [key, item] of Object.entries(value))
          result[`${name}.${key[0]!.toUpperCase()}${key.slice(1)}`] = item;
