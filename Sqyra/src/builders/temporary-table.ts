...
export function dropAndCreateTemporaryTable(
  name: string,
  columns: ReadonlyArray<TemporaryColumnDefinition>,
  dialect: "tsql" | "pgsql",
): string {
  ...
  if (primary.length === 0) throw new TypeError("A temporary table requires a primary key.");
    ...
    const type =
      column.type === "int32"
        ...
        : column.type === "date"
          ? "date"
          : dialect === "pgsql"
            ? `character varying(${column.size ?? 255})`
            : `nvarchar(${column.size ?? "MAX"})`;
  ...
  definitions.push(
    `CONSTRAINT ${quote(`PK_${name}`, dialect)} PRIMARY KEY (${primary.map((column) => quote(column.name, dialect)).join(",")})`,
  );
  if (dialect === "tsql") {
    const objectName = `[#${name.replace(/'/g, "''")}]`;
    return `IF OBJECT_ID('tempdb..${objectName}') IS NOT NULL DROP TABLE ${qName}CREATE TABLE ${qName}(${definitions.join(",")});`;
  }
