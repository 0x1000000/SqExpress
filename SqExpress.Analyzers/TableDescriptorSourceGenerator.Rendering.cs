...
public sealed partial class TableDescriptorSourceGenerator
{
        ...
        switch (issue.Kind)
        {
            case CodeGenValidationIssueKind.DuplicateColumn:
                return CreateDiagnosticsAtLocations(
                    candidate.ColumnLocationsBySqlName.TryGetValue(issue.Subject, out var duplicateColumnLocations) ? duplicateColumnLocations :
                    [
                        candidate.TableAttributeLocation
                    ],
            ...
            case CodeGenValidationIssueKind.InvalidPropertyName:
                return CreateDiagnosticsAtLocations(
                    issue.RelatedValue != null && candidate.ColumnLocationsBySqlName.TryGetValue(issue.RelatedValue, out var invalidPropertyLocations) ? invalidPropertyLocations :
                    [
                        candidate.TableAttributeLocation
                    ],
            ...
            case CodeGenValidationIssueKind.DuplicatePropertyName:
                return CreateDiagnosticsAtLocations(
                    candidate.PropertyLocationsByName.TryGetValue(issue.Subject, out var duplicatePropertyLocations) ? duplicatePropertyLocations :
                    [
                        candidate.TableAttributeLocation
                    ],
            ...
            case CodeGenValidationIssueKind.UnknownIndexColumn:
                return CreateDiagnosticsAtLocations(
                    [..candidate.IndexLocations.Where(i => i.Columns.Contains(issue.Subject)).Select(i => i.Location).DefaultIfEmpty(candidate.TableAttributeLocation)],
                    DiagnosticDescriptors.TableDescriptorUnknownIndexColumn,
                    issue.Subject,
                    issue.TableDisplayName);
            case CodeGenValidationIssueKind.DescendingColumnMustBeIndexed:
                return CreateDiagnosticsAtLocations(
                    [..candidate.IndexLocations.Where(i => i.DescendingColumns.Contains(issue.Subject)).Select(i => i.Location).DefaultIfEmpty(candidate.TableAttributeLocation)],
            ...
            case CodeGenValidationIssueKind.ForeignKeyTableNotFound:
                return CreateDiagnosticsAtLocations(
                    issue.RelatedValue != null && candidate.ColumnLocationsBySqlName.TryGetValue(issue.RelatedValue, out var foreignKeyTableLocations) ? foreignKeyTableLocations :
                    [
                        candidate.TableAttributeLocation
                    ],
            ...
            case CodeGenValidationIssueKind.ForeignKeyColumnNotFound:
                return CreateDiagnosticsAtLocations(
                    issue.RelatedValue != null && candidate.ColumnLocationsBySqlName.TryGetValue(issue.RelatedValue, out var foreignKeyColumnLocations) ? foreignKeyColumnLocations :
                    [
                        candidate.TableAttributeLocation
                    ],
    ...
    private static ImmutableArray<Diagnostic> CreateDiagnosticsAtLocations(
        ...
        params object[] args)
    {
        return [..locations.Select(location => CreateDiagnostic(descriptor, location, args))];
    }
