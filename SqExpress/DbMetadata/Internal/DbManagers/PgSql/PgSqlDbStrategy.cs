...
using SqExpress.DbMetadata.Internal.DbManagers.PgSql.Tables.InformationSchema;
using SqExpress.DbMetadata.Internal.DbManagers.PgSql.Tables.PgSchema;
using SqExpress.DbMetadata.Internal.Model;
using SqExpress.SqlExport;
using SqExpress.Syntax.Boolean;
using static SqExpress.SqQueryBuilder;

namespace SqExpress.DbMetadata.Internal.DbManagers.PgSql
{
    internal class PgSqlDbStrategy : DbStrategyBase
    {
        ...
        private class PgIndexUnNest : DerivedTableBase
        {
            ...
            {
                IndRelId = _tIndex.IndRelId.AddToDerivedTable(this);
                IndExRelId = _tIndex.IndExRelId.AddToDerivedTable(this);
                IndOption = _tIndex.IndOption.AddToDerivedTable(this);
                IndIsPrimary = _tIndex.IndIsPrimary.AddToDerivedTable(this);
                IndIsUnique = _tIndex.IndIsUnique.AddToDerivedTable(this);
                IndIsClustered = _tIndex.IndIsClustered.AddToDerivedTable(this);
            ...
            protected override IExprSubQuery CreateQuery()
            {
                return Select(
                        ...
                        _tIndex.IndExRelId,
                        _tIndex.IndOption,
                        _tIndex.IndIsPrimary,
                        _tIndex.IndIsUnique,
                        _tIndex.IndIsClustered,
                        ScalarFunctionSys(
                                "unnest",
                                ScalarFunctionSys(
                                    "ARRAY",
                                    ValueQuery(
                                        Select(
                                            ScalarFunctionSys("generate_series", 1, _tIndex.IndNkeysAtts)
                                                .As("n")
                                        )
                                    )
                                )
                                    ...
                                    ValueQuery(
                                        Select(
                                            ScalarFunctionSys(
                                                    "generate_series",
                                                    1,
                                                    ScalarFunctionSys("array_length", _table.ConKey, 1)
                                                )
                                                .As("n")
                                        )
                                    )
