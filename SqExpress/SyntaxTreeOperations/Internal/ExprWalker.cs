...
namespace SqExpress.SyntaxTreeOperations.Internal
{
    ...
    internal class ExprWalker<TCtx> : IExprVisitor<bool, WalkerContext<TCtx>>
    {
        ...
        public bool VisitExprMerge(ExprMerge expr, WalkerContext<TCtx> arg)
        {
            ...
            if (walkResult == WalkResult.Continue)
            {
                res = this.Accept("TargetTable", expr.TargetTable, argOut) && this.Accept("Source", expr.Source, argOut) && this.Accept("On", expr.On, argOut) &&
                      this.Accept("WhenMatched", expr.WhenMatched, argOut) && this.Accept("WhenNotMatchedByTarget", expr.WhenNotMatchedByTarget, argOut) &&
                      this.Accept("WhenNotMatchedBySource", expr.WhenNotMatchedBySource, argOut);
            }
        ...
        public bool VisitExprMergeOutput(ExprMergeOutput expr, WalkerContext<TCtx> arg)
        {
            ...
            if (walkResult == WalkResult.Continue)
            {
                res = this.Accept("TargetTable", expr.TargetTable, argOut) && this.Accept("Source", expr.Source, argOut) && this.Accept("On", expr.On, argOut) &&
                      this.Accept("WhenMatched", expr.WhenMatched, argOut) && this.Accept("WhenNotMatchedByTarget", expr.WhenNotMatchedByTarget, argOut) &&
                      this.Accept("WhenNotMatchedBySource", expr.WhenNotMatchedBySource, argOut) && this.Accept("Output", expr.Output, argOut);
            }
        ...
        public bool VisitExprQuerySpecification(ExprQuerySpecification expr, WalkerContext<TCtx> arg)
        {
            ...
            if (walkResult == WalkResult.Continue)
            {
                res = this.Accept("SelectList", expr.SelectList, argOut) && this.Accept("Top", expr.Top, argOut) && this.Accept("From", expr.From, argOut) &&
                      this.Accept("Where", expr.Where, argOut) && this.Accept("GroupBy", expr.GroupBy, argOut);
            }
        ...
        public bool VisitExprUpdate(ExprUpdate expr, WalkerContext<TCtx> arg)
        {
            ...
            if (walkResult == WalkResult.Continue)
            {
                res = this.Accept("Target", expr.Target, argOut) && this.Accept("SetClause", expr.SetClause, argOut) && this.Accept("Source", expr.Source, argOut) &&
                      this.Accept("Filter", expr.Filter, argOut);
            }
