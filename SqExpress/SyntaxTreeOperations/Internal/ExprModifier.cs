...
namespace SqExpress.SyntaxTreeOperations.Internal
{
    internal class ExprModifier : IExprVisitor<IExpr?, Func<IExpr, IExpr?>>
    {
        ...
        public IExpr? VisitExprMerge(ExprMerge exprIn, Func<IExpr, IExpr?> modifier)
        {
            ...
            if (!ReferenceEquals(exprIn.TargetTable, newTargetTable) || !ReferenceEquals(exprIn.Source, newSource) || !ReferenceEquals(exprIn.On, newOn) ||
                !ReferenceEquals(exprIn.WhenMatched, newWhenMatched) || !ReferenceEquals(exprIn.WhenNotMatchedByTarget, newWhenNotMatchedByTarget) ||
                !ReferenceEquals(exprIn.WhenNotMatchedBySource, newWhenNotMatchedBySource))
            {
                exprIn = new ExprMerge(targetTable: newTargetTable, source: newSource, on: newOn, whenMatched: newWhenMatched, whenNotMatchedByTarget: newWhenNotMatchedByTarget,
                    whenNotMatchedBySource: newWhenNotMatchedBySource);
            }
        ...
        public IExpr? VisitExprMergeOutput(ExprMergeOutput exprIn, Func<IExpr, IExpr?> modifier)
        {
            ...
            {
                exprIn = new ExprMergeOutput(targetTable: newTargetTable, source: newSource, on: newOn, whenMatched: newWhenMatched, whenNotMatchedByTarget: newWhenNotMatchedByTarget,
                    whenNotMatchedBySource: newWhenNotMatchedBySource, output: newOutput);
            }
        ...
        public IExpr? VisitExprQuerySpecification(ExprQuerySpecification exprIn, Func<IExpr, IExpr?> modifier)
        {
            ...
            if (!ReferenceEquals(exprIn.SelectList, newSelectList) || !ReferenceEquals(exprIn.Top, newTop) || !ReferenceEquals(exprIn.From, newFrom) || !ReferenceEquals(exprIn.Where, newWhere) ||
                !ReferenceEquals(exprIn.GroupBy, newGroupBy))
            {
        ...
        public IExpr? VisitExprUpdate(ExprUpdate exprIn, Func<IExpr, IExpr?> modifier)
        {
            ...
            var newSource = this.AcceptNullableItem(exprIn.Source, modifier);
            var newFilter = this.AcceptNullableItem(exprIn.Filter, modifier);
            if (!ReferenceEquals(exprIn.Target, newTarget) || !ReferenceEquals(exprIn.SetClause, newSetClause) || !ReferenceEquals(exprIn.Source, newSource) ||
                !ReferenceEquals(exprIn.Filter, newFilter))
            {
